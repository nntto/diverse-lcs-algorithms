...
# XとYをそれぞれ i 倍し、j 文字目まで付け加えた文字列ペアについて実験を実行する
for k in {2..10}
do
    skip_main=false
    skip_naive=false
    ...
    do
        for j in {0..0}
        do
            # 文字数 n は、XとYの文字数の i 倍 + j
            n=$((9 * $i + $j))
            echo "n = $n, k = $k" >> $results_file
            echo "n = $n, k = $k"

            # 文字列ペアの作成
            str_X=$(printf "%0.s$X" $(seq 1 $i))${X:0:$j}
            str_Y=$(printf "%0.s$Y" $(seq 1 $i))${Y:0:$j}
            ...
            if [ "$skip_main" = false ]; then
                ...
                if [ $exit_status_main -eq 124 ]; then
                    echo "Timeout occurred for n=$n, k=$k" >> $results_file
                    skip_main=true
                fi
            else 
